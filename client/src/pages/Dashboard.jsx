...
import { Search, Grid, List as ListIcon, Plus, Menu, Home, Calendar as CalendarIcon, LogOut, X } from 'lucide-react';

export function Dashboard({ user, onLogout, onUpdateUser, showToast, setServerError }) {
  // Notes and UI State
  ...
  // Fetch notes when user logs in / mount
  useEffect(() => {
    if (user) {
      fetchNotes();
      fetchFolders();
  ...
  return (
    <div className="min-h-screen flex bg-[#f9f8f5] relative pb-20 lg:pb-0 w-full">
      {/* Sidebar Navigation */}
      <Sidebar
        notes={notes}
        user={user}
              ...
              <div className="flex bg-white p-1 rounded-full border border-slate-200/60 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all cursor-pointer ${
                    viewType === 'grid' ? 'bg-indigo-50 text-indigo-700 shadow-xs' : 'text-slate-400 hover:text-slate-650'
                  }`}
                  title="Grid View"
                >
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all cursor-pointer ${
                    viewType === 'list' ? 'bg-indigo-50 text-indigo-700 shadow-xs' : 'text-slate-400 hover:text-slate-650'
                  }`}
                  title="List View"
                >
